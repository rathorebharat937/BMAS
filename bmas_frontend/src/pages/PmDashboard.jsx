...
function PmDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans relative overflow-hidden flex flex-col justify-between">
      ...
      <div className="absolute top-0 -right-4 w-96 h-96 bg-indigo-500 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-blob [animation-delay:2s]"></div>

      {/* Header */}
      <header className="sticky top-0 z-55 backdrop-blur-md bg-slate-950/60 border-b border-slate-800/80">
        <div className="max-w-7xl mx-auto px-6 h-18 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-pink-500 via-rose-500 to-amber-500 flex items-center justify-center font-black text-xl tracking-wider shadow-lg">
              P
            </div>
            <div>
              ...
              <span className="text-xs block text-pink-400 font-medium uppercase tracking-wider">
                Manager Workspace
              </span>
            </div>
          </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-16 flex-1 flex flex-col justify-center relative z-10 w-full">
        <div className="bg-slate-900/40 border border-slate-800 p-8 md:p-12 rounded-3xl shadow-xl">
          <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-pink-500/10 border border-pink-500/20 text-pink-400 text-xs font-bold uppercase tracking-wider mb-6">
            ✔ Authorization Authenticated
          </span>
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight text-white mb-4">
            Welcome back, <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-400 to-rose-400">{user?.name}</span>
          </h1>
          <p className="text-slate-400 text-base md:text-lg mb-8 leading-relaxed">
            Welcome to the Bug Management & Assignment System. As a <strong className="text-white">Project Manager</strong>, you can assign bug reports, monitor current team workloads, manage milestones, and analyze quality trends.
          </p>
          ...
          <div className="border-t border-slate-800/80 pt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <span className="text-sm font-semibold text-pink-400">{user?.role || user?.activeRole}</span>
            </div>
          </div>
        </div>
      </main>
      ...
      <footer className="border-t border-slate-900/80 py-6 text-center text-slate-650 text-xs bg-slate-950">
        BMAS Portal &copy; {new Date().getFullYear()} - Dedicated PM Workspace
      </footer>
    </div>
  );
}
