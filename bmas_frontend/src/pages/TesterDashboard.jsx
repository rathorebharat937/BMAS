  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans relative overflow-hidden flex flex-col justify-between">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-md bg-slate-950/60 border-b border-slate-800/80">
        <div className="max-w-7xl mx-auto px-6 h-18 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-emerald-400 via-teal-500 to-indigo-600 flex items-center justify-center font-black text-xl tracking-wider shadow-lg">
              T
            </div>
            <div>
              ...
              <span className="text-xs block text-emerald-400 font-medium uppercase tracking-wider">
                Tester Workspace
              </span>
            </div>
          </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-16 flex-1 flex flex-col justify-center relative z-10 w-full">
        <div className="bg-slate-900/40 border border-slate-800 p-8 md:p-12 rounded-3xl shadow-xl">
          ...
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight text-white mb-4">
            Welcome back, <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-indigo-400">{user?.name}</span>
          </h1>
          <p className="text-slate-400 text-base md:text-lg mb-8 leading-relaxed">
            Welcome to the Bug Management & Assignment System. As a <strong className="text-white">Tester</strong>, you can create new bug tickets, run automated quality validations, and evaluate developers' patches.
          </p>
          ...
          <div className="border-t border-slate-800/80 pt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <span className="text-sm font-semibold text-emerald-400">{user?.role || user?.activeRole}</span>
            </div>
          </div>
        </div>
      </main>
      ...
      <footer className="border-t border-slate-900/80 py-6 text-center text-slate-650 text-xs bg-slate-950">
        BMAS Platform &copy; {new Date().getFullYear()} - Dedicated Testing Workspace
      </footer>
    </div>
  );
}
